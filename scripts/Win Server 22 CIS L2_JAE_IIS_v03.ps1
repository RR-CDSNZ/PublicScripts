##########################################################################################################
# HYB - Datto Agent and Timezone Setup
##########################################################################################################
...
Add-WindowsCapability -Online -Name "Language.Fonts~~~ja-JP~0.0.1.0"

# Install English UK Language Pack
Add-WindowsCapability -Online -Name "Language.Basic~~~en-GB~0.0.1.0"

# Set English UK as default Language
Set-WinUserLanguageList -LanguageList en-GB -Force


##########################################################################################################
# HYB - IIS Server Roles Install
##########################################################################################################

# Module Import
Import-Module ServerManager

# List of Server Roles\features to enable
$featuresList = @(
...
# Check if feature is enabled and if not, then enable it
foreach ($featureName in $featuresList) {
$featureState = Get-WindowsFeature -Name $featureName
...
if ($featureState.InstallState -eq "Installed") {
    ...
    $isEnabled = $false
    Write-Output "$featureName is not installed, starting install command"
    try {
        Install-WindowsFeature -Name $featureName -IncludeAllSubFeature
        Write-Output "$featureName is now ENABLED"
    } catch {
        Write-Output "ERROR failed to enable $featureName : $($_.Exception.Message)"
    }
}
}
...
$NewLocalAdmin = "azure_admin" # Active admin account username (Local admin account that will be used to manage the server. Account will be active after script is run. This is not a prefix. It's the full account username)
...
$AttackSurfaceReductionExclusions = @(
    # To check which Paths have been added use:
    # Get-MpPreference | Select-Object -Property AttackSurfaceReductionExclusions -ExpandProperty AttackSurfaceReductionExclusions
    # Folder Example
    #"C:\Program Files\RMM"
...
$ExecutionList = @(
    ...
    #"PerformRouterDiscovery",                                           #18.4.7 - DISABLED - can lead to DoS
    ...
    "EnableDNSOverDoH",                                                 #18.5.4.1 (2023.01.27 - added support) - Changed value to 2 for compatibility
    ...
    #"PeernetDisabled",                                                  #18.5.10.2 - DISABLED - causes network issues
    ...
    #"ProcessCreationIncludeCmdLine",                                    #18.8.3.1 - DISABLED
    ...
    #"WinRSAllowRemoteShellAccess",                                     #18.9.103.1 (2023.01.27 - added to default configuration) - DISABLED, affects IIS Service install
    ...
    "AuditMinimumPasswordLength",                                       # HYB - NIST SP 800 171 R2 - Audit Windows machines that do not restrict the minimum password length to specified number of characters
    "AddDefenderExclusions",                                            # HYB - Add Defender Exclusions
    "EnableDefenderExploitGuard",                                       # HYB - Enable Defender Exploit Guard
    "ConfigureDefenderSignatureUpdates",                                # HYB - Windows Defender Auto Signature updates
...
function DenyNetworkAccess {
    ...
    if ($AllowRDPFromLocalAccount -eq $true) {
        SetUserRight "SeDenyNetworkLogonRight" ($addlDenyUsers,$($SID_GUESTS))
    }
    else {
        SetUserRight "SeDenyNetworkLogonRight" ($addlDenyUsers,$SID_LOCAL_ACCOUNT,$($SID_GUESTS))
    }
}
...
function EnableDNSOverDoH {
    ...
    SetRegistry "HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient" "DoHPolicy" "2" $REG_DWORD
}
...
########################################################################
# Function Definitions added by HYB
########################################################################

# ----------------------------------
# HYB - NIST SP 800 171 R2 - Audit Windows machines that do not restrict the minimum password length to specified number of characters
function AuditMinimumPasswordLength {

    Write-Info "HYB - NIST SP 800 171 R2 - Audit Windows machines that do not restrict the minimum password length to specified number of characters"
    SetSecurityPolicy "MACHINE\System\CurrentControlSet\Control\SAM\MinimumPasswordLengthAudit" (,"4,14")
}
...
# ----------------------------------
# HYB - Windows Defender Exploit Guard should be enabled on machines
function EnableDefenderExploitGuard {
   # TODO
}
...
# ----------------------------------
# HYB - Enable Security Event auditing
function EnableSecurityAuditing {
    # Audit Logon Events
    auditpol /set /subcategory:"Logon" /success:enable /failure:enable
    ...
    # Audit System Integrity (for Local System behavior)
    auditpol /set /subcategory:"System Integrity" /success:enable /failure:enable

    # Audit Credential Validation (for PKU2U)
    auditpol /set /subcategory:"Credential Validation" /success:enable /failure:enable

    # Audit SAM (for LAN Manager)
    auditpol /set /subcategory:"SAM" /success:enable /failure:enable

    # Audit Directory Service Access (for LDAP client)
